...
os.environ["WANDB_API_KEY"] = "95acdd82b41f3fb61dd914d9bb87599ccf73c561"
...
def main_worker(gpu, ngpus_per_node, args):
    wandb.init(
        project="CVPR24-LiteMedSAM",
        config={
        "Model": "LiteMedSAM",
        "dataset": "CVPR-2024-Workshop-MedSAM",
        }
    )
    ...
    medsam_lite_mask_decoder = MaskDecoder(
        ...
        transformer=TwoWayTransformer(
            depth=1,  # 줄임
            embedding_dim=256,  # 각 레이어에 맞게 조정
            mlp_dim=1024,  # 줄임
            num_heads=8,  # 줄임
        ),
        transformer_dim=256,  # 각 레이어에 맞게 조정
        iou_head_depth=2,  # 줄임
        iou_head_hidden_dim=256,  # 각 레이어에 맞게 조정
    )
    ...
    #     medsam_lite_model.load_state_dict(medsam_lite_checkpoint, strict=False) ######## true -> false 구조 달라지는거 무시.
    def load_partial_weights(model, pretrained_path):
        ...
        return list(pretrained_dict.keys()), missing_keys, unexpected_keys
    
    loaded_keys, missing_keys, unexpected_keys = load_partial_weights(medsam_lite_model,args.pretrained_checkpoint)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        #wandb.log({"Loss": float(epoch_loss_reduced)})
